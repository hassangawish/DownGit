...
# Dynamically get current user's home directory (works on any Mac user)
USER_HOME="$HOME"
...
# =========================
# APK SMART INSTALL (محسن - داونجريد + توقيع)
# =========================
...
central() {
  ...
  for user in "${USERS[@]}"; do
    ...
    ADB_CMD shell ime enable --user "$user" \
      com.google.android.inputmethod.latin/com.android.inputmethod.latin.LatinIME >/dev/null 2>&1 || true
  ...
  ADB_CMD shell am start -a android.intent.action.BYD_APPSTARTMANAGEMENT

  for i in {1..15}; do

    XML=$(ADB_CMD exec-out uiautomator dump /dev/tty 2>/dev/null)

    if echo "$XML" | grep -q 'text="microG Services"'; then

      AFTER=$(echo "$XML" | grep -o 'text="microG Services".*')

      CHECKED=$(echo "$AFTER" | grep -o 'select_button[^/]*/>' | head -1 | grep -o 'checked="[^"]*"')

      echo "DEBUG: CHECKED=$CHECKED"

      if [ "$CHECKED" = 'checked="true"' ]; then
        echo "microG Services = TRUE -> changing to FALSE"
        ADB_CMD shell input tap 960 627
        break

      elif [ "$CHECKED" = 'checked="false"' ]; then
        echo "microG Services is already FALSE -> nothing to do"
        break
      fi
    fi

    ADB_CMD shell input swipe 950 800 950 350 250
...
zeekr() {
  ...
  ADB_CMD shell ime enable \
    com.google.android.inputmethod.latin/com.android.inputmethod.latin.LatinIME >/dev/null 2>&1 || true

  ADB_CMD shell ime set \
    com.google.android.inputmethod.latin/com.android.inputmethod.latin.LatinIME >/dev/null 2>&1 || true

  ADB_CMD shell pm grant \
    jp.co.c_lis.ccl.morelocale android.permission.CHANGE_CONFIGURATION || true
  ...
  ADB_CMD shell settings put secure \
    ...
    ace.jun.simplecontrol/ace.jun.simplecontrol.service.AccService || true
...
Zeekr9x() {
  ...
  ADB_CMD shell ime enable --user 0 \
    com.google.android.inputmethod.latin/com.android.inputmethod.latin.LatinIME >/dev/null 2>&1 || true

  ADB_CMD shell ime set --user 0 \
    com.google.android.inputmethod.latin/com.android.inputmethod.latin.LatinIME >/dev/null 2>&1 || true

  ADB_CMD shell ime enable --user 10 \
    com.touchtype.swiftkez/com.touchtype.KeyboardService >/dev/null 2>&1 || true

  ADB_CMD shell ime set --user 10 \
    com.touchtype.swiftkez/com.touchtype.KeyboardService >/dev/null 2>&1 || true
